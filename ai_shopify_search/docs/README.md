...
### 0. Environment Setup (Na fresh clone)
...
# Of check of alle environment variables correct zijn ingesteld
...
# Bewerk .env met je echte API keys
...
**⚠️ Belangrijk:** Het `.env` bestand wordt niet in Git opgeslagen voor veiligheid. Na een fresh clone moet je dit opnieuw aanmaken.

**Automatische setup:**
```bash
# Gebruik het setup script om automatisch een .env bestand aan te maken
python setup_env.py
```
...
**Probleem:** `.env` bestand verdwenen na Git pull/clone
**Oplossing:**
```bash
...
# Of handmatig
cp env.example .env
# Vul daarna je API keys in
```

**Probleem:** Environment variables worden niet geladen
**Oplossing:**
```bash
# Check of alle variabelen correct zijn ingesteld
python3 setup_env.py check
...
- Gebruik altijd `env.example` als template
- Bewaar API keys veilig en deel ze niet
- Roteer API keys regelmatig

### Development Workflow
1. **Na fresh clone:** `python3 setup_env.py`
2. **Bij nieuwe variabelen:** Update `env.example`
3. **Voor deployment:** Gebruik environment variables van je hosting platform
4. **Voor testing:** Gebruik test-specifieke `.env.test` bestand

### Backup Strategie
- Bewaar een veilige kopie van je API keys
- Documenteer welke services welke keys gebruiken
- Overweeg een password manager voor team projects
